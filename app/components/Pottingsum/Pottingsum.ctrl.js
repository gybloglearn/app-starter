    ...
    function load() {
      ...
      SumserviceService.get(vm.startdate, vm.enddate, vm.mch).then(function (response) {
        ...
        for (var i = 0; i < response.data.length; i++) {
          ...
          //különbség1<130
          if (vm.data[i].PT_URET_PREP_A_DT > 0 && vm.data[i].PT_POST_URET_A_DT > 0) {
            vm.data[i].difference_PT_URET_PREP_A_DT_PT_POST_URET_A_DT = (vm.data[i].PT_POST_URET_A_DT - vm.data[i].PT_URET_PREP_A_DT) / (60 * 1000);
          }
          else {
            vm.data[i].difference_PT_URET_PREP_A_DT_PT_POST_URET_A_DT = 0;
          }
          //különbség2<150
          if (vm.data[i].PT_POST_URET_A_DT > 0 && vm.data[i].PT_ROT_DT) {
            vm.data[i].difference_PT_POST_URET_A_DT_PT_ROT_DT = (vm.data[i].PT_ROT_DT - vm.data[i].PT_POST_URET_A_DT) / (60 * 1000);
          }
          else {
            vm.data[i].difference_PT_POST_URET_A_DT_PT_ROT_DT = 0;
          }
          //különbség3<130
          if (vm.data[i].PT_URET_PREP_F_DT > 0 && vm.data[i].PT_POST_URET_F_DT > 0) {
            vm.data[i].difference_PT_URET_PREP_F_DT_PT_POST_URET_F_DT = (vm.data[i].PT_POST_URET_F_DT - vm.data[i].PT_URET_PREP_F_DT) / (60 * 1000);
          }
          else {
            vm.data[i].difference_PT_URET_PREP_F_DT_PT_POST_URET_F_DT = 0;
          }
          //különbség4<400
          if (vm.data[i].PT_POST_URET_F_DT > 0 && vm.data[i].PT_OUT > 0) {
            vm.data[i].difference_PT_POST_URET_F_DT_PT_OUT = (vm.data[i].PT_OUT - vm.data[i].PT_POST_URET_F_DT) / (60 * 1000);
          }
          else {
            vm.data[i].difference_PT_POST_URET_F_DT_PT_OUT = 0;
          }
