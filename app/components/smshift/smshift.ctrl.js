  ...
  function Controller(weeklyService, $cookies, $state, $rootScope, $filter) {
    ...
    function createdates() {
      ...
      for (var i = 0; i <= differencedate; i++) {
        ...
        vm.days.push({
          date: vm.dates[i],deszak: '', ejszak:'', dejoaeq: 0, ejjoaeq:0, dettlaeq: 0, ejttlaeq:0, dejolap: 0, ejjolap: 0, dettllap: 0, ejttllap: 0, delapselejt: 0, ejlapselejt:0, deterv: 0, deszer: 0, demusz: 0, dettlido: 0, ejterv: 0, ejszer: 0, ejmusz: 0, ejttlido: 0
        });
      }
      callsm();
    }
        ...
        weeklyService.getsm(vm.startdate, $filter('date')(new Date(vm.enddate).getTime() + 24 * 60 * 60 * 1000, "yyyy-MM-dd"), vm.sheetmakers[i]).then(function (response) {
          ...
          for (var j = 0; j < response.data.length; j++) {
            ...
            for (var k = 0; k < vm.days.length; k++) {
              ...
              vm.days[k].dettlido = vm.sheetmakers.length * ido;
              vm.days[k].ejttlido = vm.sheetmakers.length * ido;
              if ($filter('date')(new Date(response.data[j].days), "yyyyMMdd") == vm.days[k].date && response.data[j].category == "GOOD") {
                if(response.data[j].shiftnum == 1) {
                  vm.days[k].dejoaeq += response.data[j].aeq;
                  vm.days[k].dejolap += response.data[j].amount;
                  vm.days[k].deszak = $filter('shift')(response.data[j].shiftnum, $filter('date')(new Date(response.data[j].days), "yyyy-MM-dd"));
                } else  {
                  vm.days[k].ejjoaeq += response.data[j].aeq;
                  vm.days[k].ejjolap += response.data[j].amount;
                  vm.days[k].ejszak = $filter('shift')(response.data[j].shiftnum, $filter('date')(new Date(response.data[j].days), "yyyy-MM-dd"));
                }
              } else if ($filter('date')(new Date(response.data[j].days), "yyyyMMdd") == vm.days[k].date && response.data[j].category == "TOTAL") {
                if(response.data[j].shiftnum == 1){
                  vm.days[k].dettlaeq += response.data[j].aeq;
                  vm.days[k].dettllap += response.data[j].amount;
                  vm.days[k].deszak = $filter('shift')(response.data[j].shiftnum, $filter('date')(new Date(response.data[j].days), "yyyy-MM-dd"));
                } else {
                  vm.days[k].ejttlaeq += response.data[j].aeq;
                  vm.days[k].ejttllap += response.data[j].amount;
                  vm.days[k].ejszak = $filter('shift')(response.data[j].shiftnum, $filter('date')(new Date(response.data[j].days), "yyyy-MM-dd"));
                }
              }
            }
    ...
    function setCh(ser) {
      ...
      if (ser.length == vm.sheetmakers.length) {
        ...
        for (var x = 0; x < vm.days.length; x++) {
          ...
          vm.days[x].demusz = parseFloat($filter('sumField')($filter('filter')(vm.filedatas, { d: vm.days[x].date, Shift_ID: vm.days[x].date + '01', Ev_Group: "Muszaki technikai okok" }), "Event_time")) / 60;
          vm.days[x].ejmusz = parseFloat($filter('sumField')($filter('filter')(vm.filedatas, { d: vm.days[x].date, Shift_ID: vm.days[x].date + '03', Ev_Group: "Muszaki technikai okok" }), "Event_time")) / 60;
          vm.days[x].deszer = parseFloat($filter('sumField')($filter('filter')(vm.filedatas, { d: vm.days[x].date, Shift_ID: vm.days[x].date + '01', Ev_Group: "Szervezesi veszteseg" }), "Event_time")) / 60;
          vm.days[x].ejszer = parseFloat($filter('sumField')($filter('filter')(vm.filedatas, { d: vm.days[x].date, Shift_ID: vm.days[x].date + '03', Ev_Group: "Szervezesi veszteseg" }), "Event_time")) / 60;
          vm.days[x].deterv = parseFloat($filter('sumField')($filter('filter')(vm.filedatas, { d: vm.days[x].date, Shift_ID: vm.days[x].date + '01', Ev_Group: "Tervezett veszteseg" }), "Event_time")) / 60;
          vm.days[x].ejterv = parseFloat($filter('sumField')($filter('filter')(vm.filedatas, { d: vm.days[x].date, Shift_ID: vm.days[x].date + '03', Ev_Group: "Tervezett veszteseg" }), "Event_time")) / 60;
        }
        ...
        for (var j = 0; j < vm.days.length; j++) {
          ...
          demuszperc.push({ cat: vm.days[j].date, y: vm.days[j].demusz / vm.days[j].dettlido * 100, min: vm.days[j].demusz });
          deszerperc.push({ cat: vm.days[j].date, y: vm.days[j].deszer / vm.days[j].dettlido * 100, min: vm.days[j].deszer });
          detervperc.push({ cat: vm.days[j].date, y: vm.days[j].deterv / vm.days[j].dettlido * 100, min: vm.days[j].deterv });
          deelerperc.push({ cat: vm.days[j].date, y: ((vm.days[j].dettlido - vm.days[j].deterv - vm.days[j].deszer - vm.days[j].demusz) / vm.days[j].dettlido) * 100, min: vm.days[j].dettlido - vm.days[j].demusz - vm.days[j].deszer - vm.days[j].deterv });
          ejmuszperc.push({ cat: vm.days[j].date, y: vm.days[j].ejmusz / vm.days[j].ejttlido * 100, min: vm.days[j].ejmusz });
          ejszerperc.push({ cat: vm.days[j].date, y: vm.days[j].ejszer / vm.days[j].ejttlido * 100, min: vm.days[j].ejszer });
          ejtervperc.push({ cat: vm.days[j].date, y: vm.days[j].ejterv / vm.days[j].ejttlido * 100, min: vm.days[j].ejterv });
          ejelerperc.push({ cat: vm.days[j].date, y: ((vm.days[j].ejttlido - vm.days[j].ejterv - vm.days[j].ejszer - vm.days[j].ejmusz) / vm.days[j].ejttlido) * 100, min: vm.days[j].ejttlido - vm.days[j].ejmusz - vm.days[j].ejszer - vm.days[j].ejterv });
          dejo.push({ cat: vm.days[j].date, y: vm.days[j].dejoaeq, szak: vm.days[j].deszak });
          ejjo.push({ cat: vm.days[j].date, y: vm.days[j].ejjoaeq, szak: vm.days[j].ejszak });
        }
        ...
        vm.dayavailconfig = {
          ...
          plotOptions: { line: {stacking: 'normal'}, column: { stacking: 'normal', dataLabels: {enabled: true, allowOverlap: false, zIndex:1, format: "<span class='back' style='color:rgba(0,0,0,1)'>{point.y:.2f} %</span>", useHTML: true} }},
          ...
          series: [
            { name: "Műszaki DE", color: "rgba(255,0,0,.5)", data: demuszperc, yAxis: 0, stack: 'DE' },
            { name: "Szervezési DE", color: "rgba(150,150,150,.5)", data: deszerperc, yAxis: 0, stack: 'DE' },
            { name: "Tervezett DE", color: "rgba(50,100,200,.5)", data: detervperc, yAxis: 0, stack: 'DE' },
            { name: "Elérhetőség DE", color: "rgba(150,200,100,.5)", data: deelerperc, yAxis: 0, stack: 'DE' },
            {
              name: "Össz Termelés DE", type: "line", color: "rgb(150,200,255)", data: dejo, yAxis: 1, marker: { enabled: true, color: "rgb(150,200,255)" }, stack: 'DE',
              tooltip: { headerFormat: '<span style="font-size: 10px"><b>{point.key}</b></span><br/>', pointFormat: '<span>--- {series.name}: <span style="color:{series.color};font-weight:bold">{point.y:.0f} AEQ</span></span> ({point.szak})<br/>' },
            },
            { name: "Műszaki ÉJ", color: "rgba(205,0,0,.5)", data: ejmuszperc, yAxis: 0, stack: 'EJ' },
            { name: "Szervezési ÉJ", color: "rgba(100,100,100,.5)", data: ejszerperc, yAxis: 0, stack: 'EJ' },
            { name: "Tervezett ÉJ", color: "rgba(0,50,100,.5)", data: ejtervperc, yAxis: 0, stack: 'EJ' },
            { name: "Elérhetőség ÉJ", color: "rgba(50,150,100,.5)", data: ejelerperc, yAxis: 0, stack: 'EJ' },
            {
              name: "Össz Termelés ÉJ", type: "line", color: "rgb(50,100,155)", data: ejjo, yAxis: 1, marker: { enabled: true, color: "rgb(50,100,155)" }, stack: 'EJ',
              tooltip: { headerFormat: '<span style="font-size: 10px"><b>{point.key}</b></span><br/>', pointFormat: '<span>--- {series.name}: <span style="color:{series.color};font-weight:bold">{point.y:.0f} AEQ</span></span> ({point.szak})<br/>' },
            }
          ]
        };
